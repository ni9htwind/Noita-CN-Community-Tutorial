    ...
    while dialog_system.is_open do
      ...
      if dialog.fade_in_portrait > -1 then
        do
          ...
          local portrait_width, portrait_height = GuiGetImageDimensions( gui, dialog.message.portrait, 1 )
          GuiImage(gui, 2, x, y, dialog.message.portrait, 1, 64 / portrait_width, 64 / portrait_height, 0, GUI_RECT_ANIMATION_PLAYBACK.Loop, message.animation or "")
        end
      ...
      for i, line in ipairs(dialog.lines) do
        ...
        for i2, char_data in ipairs(line) do
          ...
          do
            local _,_,_,px,_,pw,_ = GuiGetPreviousWidgetInfo(gui)
            if px + pw >= screen_width/2 + width/2 then
